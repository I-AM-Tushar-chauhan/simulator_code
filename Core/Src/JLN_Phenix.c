			...
			(JLN_RxHeader.FDFormat == FDCAN_CLASSIC_CAN))
		{
			...
			switch(JLN_RxHeader.Identifier)
			{
				case 0x18FF50E5:	//	Voltage and Current
					...
					JLN_canTxData[0] = 0x03;//((uint16_t)((float)CellCount*3.25*10) >> 8);//VOLTAGE HIGH
					JLN_canTxData[1] = 0xE8;//((uint16_t)((float)CellCount*3.25*10) & 0xff);//VOLTAGE LOW
					JLN_canTxData[2] = 0x01;//CURRENT HIGH
					JLN_canTxData[3] = 0xF4;//CURRENT LOW	25A current
